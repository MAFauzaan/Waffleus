...
const LaporanPemesanan = (props) => {
    const classes = props.style
    const orderTerkirim = props.orderTerkirim

    const [ inputLaporanPemesanan, setInputLaporanPemesanan ] = useState({
        tanggalAwal: "", 
        tanggalAkhir: ""
      })
      
    const [ foundLaporanPemesanan, setFoundLaporanPemesanan ] = useState([])
    const flattenedLaporanPemesanan = foundLaporanPemesanan.flat()    


      //Functions
      const findLaporanPemesanan =  (beginning, end) => {
        let dateArray =[];
        ...
        while( currentDate <= endDate ) {
          ...
          const foundOrder = orderTerkirim.filter(order => order.orderDate === curr)
         ...
         setFoundLaporanPemesanan(filteredArray)
    
      }

      const resetLaporanPemesanan = () => {
        setFoundLaporanPemesanan([])
        setInputLaporanPemesanan({
         tanggalAwal: "", 
         tanggalAkhir: ""
       })
     }
     ...
     const convert = () => {
      ...
      const tanggalAwal = `Tanggal Awal: ${inputLaporanPemesanan.tanggalAwal}`
      const tanggalAkhir = `Tanggal Akhir: ${inputLaporanPemesanan.tanggalAkhir}`
      ...
      const header = [["No", "Order Id", "Nama Pelanggan", "Tanggal Pesan", "Nama Produk", "Kuantitas Produk", "Harga(Rp)", "Kuantitas", "Diskon(Rp)" ,"Total Harga(Rp)", "Status"]]
    
      const data = flattenedLaporanPemesanan.map((order, index) =>  [
        index+1,  
        order._id,
        order.user.username, 
        order.orderDate, 
      ...
      const body = [...data, ["", "","", "", "", "", "", "", "", "GrandTotal", flattenedLaporanPemesanan.reduce((b, a) => b + +a.grandTotal, 0)]]
      ...
      doc.save('laporanPemesanan.pdf')
    }
   

    return(
      <div>
      <h1 className={classes.h1}>Laporan Pemesanan</h1>
      ...
      <div className={classes.insertContainer}>
         <div className={classes.insertItem}>
            ...
            <input 
                ...
                onChange={(e) => setInputLaporanPemesanan({...inputLaporanPemesanan, tanggalAwal: e.target.value})}
                value={inputLaporanPemesanan.tanggalAwal}
            />
         </div>
         ...
         <div className={classes.insertItem} onMouseEnter={() => "Bulan/Tanggal/Tahun"}>
            ...
            <input 
                type = "date"
                className={`${classes.insertComponent} ${classes.input}`} 
                onChange={(e) => setInputLaporanPemesanan({...inputLaporanPemesanan, tanggalAkhir: e.target.value})}
                value={inputLaporanPemesanan.tanggalAkhir}
            />                 
          </div>

        <Button 
            className={classes.promoSubmit}
            onClick={() => findLaporanPemesanan(inputLaporanPemesanan.tanggalAwal, inputLaporanPemesanan.tanggalAkhir)}
        >
          Tampilkan
        </Button>
       <Button className={classes.refresh} onClick={resetLaporanPemesanan}><Refresh /></Button>
      ...
      <table style={{width: '100%', background: '#ffff'}} className={classes.table}>
        <thead>
        ...
        </thead>
          {
                ...
                orderTerkirim.map((order, index) => {
                  return(
                    <tr key={index+1}>
                      ...
                      <td className={classes.table}>{order.totalQuantity}</td>

                      <td className={classes.table}>{order.potonganHarga}</td>

                      <td className={classes.table}>{`${order.grandTotal}(dsc${order.potonganHarga})`}</td>
                      
                      <td className={classes.table}>{order.deliveryStatus}</td>
                    </tr>
                  )
                })
              }
                 <tr>
                      <td colSpan="9" className={classes.tableGrandTotal}>Grand Total:</td>
            ...
            <tbody>
            {
              flattenedLaporanPemesanan.map((order, index) => {
                return(
                  <tr key={index} className={classes.table}>
                    <td className={classes.table}>{index+1}</td>
                    <td className={classes.table}>{order._id}</td>
                    <td className={classes.table}>{order.user.username}</td>
                    <td className={classes.table}>{order.orderDate}</td>
            ...
            <tr>
                <td colSpan="9" className={classes.tableGrandTotal}>Grand Total:</td>
                <td colSpan="2" className={`${classes.table} ${classes.total}`}>Rp.{flattenedLaporanPemesanan.reduce((b, a) => b + +a.grandTotal, 0)},00</td>
            </tr>
          </tbody>
          } 
      </table>
...
export default LaporanPemesanan